...
```{r read-recession-data, echo=TRUE}
...
```{r p1-hpi-mort, fig.cap="Figure 3"}
p1_hpi_mort <- ggplot()+ 
...
p2 <- hpi_mort %>% 
  ...
  ylab("US Average HPI") +
  ggtitle("Linear model for mortgage rate and HPI")

ggplotly(p2)
```

```{r hpi-mort-lm}
hpi_mort_lm <- lm(us_avg ~ lag(`mortgage rates`), data = hpi_mort)
summary(hpi_mort_lm)
```
...
```{r recession-kable}
recession_dates1 %>%
  kable(caption = "Cleaned Recession Data") %>% 
  kable_styling(bootstrap_options = c("striped", "hover"))
```

```{r p2-hpi-recession, fig.cap="Figure 5"}
...
```{r p3_mortgage_recession, fig.cap="Figure 6"}
