...
data_hpi %>% 
  head(5) %>% 
  kable(caption = "Freddie Mac HPI Dataset") %>% 
...
data_mortage %>% 
  head(5) %>% 
  kable(caption = "Freddie Mac Mortgage Dataset") %>% 
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
recession_data %>% 
  head(1) %>% 
  kable(caption = "Recession Dataset") %>% 
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
state_data_hpi <- state_hpi_abb %>% dplyr::select("date", "region", "state", "price_index", "us_avg") %>% group_by(date,region,us_avg) %>% summarise(price_index = median(price_index))

p1 <- state_data_hpi %>%
  ggplot()+ 
  geom_line(data=state_data_hpi, aes(x=date, y=price_index), color="red") +
  geom_line(data=state_data_hpi, aes(x=date, y=us_avg), color="green") + 
  facet_wrap( ~ region) +
  xlab("Date") +
